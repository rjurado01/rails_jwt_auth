...
describe RailsJwtAuth::Jwt::Request do
  ...
  describe '#valid?' do
    context 'when all is valid' do
      it 'returns true' do
        ...
        request = Rack::Request.new('HTTP_AUTHORIZATION' => "Bearer #{jwt}")
    ...
    context 'when verification fails' do
      ...
      it 'returns false' do
        ...
        request = Rack::Request.new('HTTP_AUTHORIZATION' => "Bearer #{invalid}")
  ...
  describe '#header' do
    it 'returns jwt header' do
      ...
      jwt_request = RailsJwtAuth::Jwt::Request.new(request)
      expect(jwt_request.header).to eq('alg' => 'HS256')
    end
  end
