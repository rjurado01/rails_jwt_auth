        ...
        context 'with existing user' do
          context 'with pending invitation' do
            ...
            it 'resets invitation_sent_at' do
              ...
              Timecop.freeze(DateTime.current + 30.days) do
                "#{orm}User".constantize.invite! email: user.email
                expect(user.reload.invitation_sent_at.to_datetime.to_i)
                  .to eq(Time.current.utc.to_datetime.to_i)
              end

              expect(user.reload.invitation_sent_at.to_datetime.to_i)
                .to_not eq(Time.current.utc.to_datetime.to_i)
              expect(user.reload.invitation_sent_at.to_datetime.to_i)
                .to eq((Time.current.utc.to_datetime + 30.days).to_datetime.to_i)
              Timecop.return
            end
      ...
      describe '#accept_invitation!' do
        context 'with invited user' do
          ...
          it 'sets invitation_accepted_at' do
            expect(invited_user.invitation_accepted_at).to eq(Time.current.utc)
          end
        end
